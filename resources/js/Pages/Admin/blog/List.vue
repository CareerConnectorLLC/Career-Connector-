    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            <div class="card-title mb-0">
              ...
              <small class="text-muted">{{ store.pagination.total}} Blogs</small>
            </div>
            <div class="dropdown">
              <Link :href="route('admin.blog.create')" class="btn rounded-pill btn-outline-primary waves-effect">+ Add Blog</Link>
            </div>
          </div>
        <div class="card-body">
            <DataTable :value="store.list" lazy showGridlines @page="store.onPage" @sort="store.onSort"
                ...
                <Column field="title" header="Blog Title" sortable />
                <Column field="category" header="Category" unstyle>
                    <template #body="slotData">
                        <div class="d-flex gap-1">
                            <span class="badge bg-label-primary">{{ slotData.data.category.name }}</span>
                        </div>
                    </template>
                </Column>
                <Column field="full_name" header="Author" sortable >
                    <template #body="slotData">
                        <div class="d-flex flex-wrap">
                            <div>
                                <h6 class="mb-0">
                                    {{ slotData.data.user?.full_name }}
                                </h6>
                            </div>
                        </div>
                    </template>
                    ...
                    <template #body="slotData">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                ...
                                <Link class="dropdown-item" :href="route('admin.blog.edit', slotData.data.id)"><i class="ti ti-edit me-1"></i> Edit</Link>
...
<script setup>
    ...
    onMounted(() => {
        nextTick(() => {
            emit.emit('pageName', 'Blog Management', [{ title: 'Blogs', routeName: "" }]);
        });
        store.getBlogs();
    });
