    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between">
            <div class="card-title mb-0">
                ...
                <small class="text-muted">{{ store.pagination.total}} Bookings</small>
            </div>
            <div class="dropdown">
                <Link :href="route('admin.service-providers.create')"
                    class="btn rounded-pill btn-outline-primary waves-effect">+ New Booking</Link>
            </div>
        </div>
        <div class="card-body">
            <DataTable :value="store.list" lazy showGridlines @page="store.onPage" @sort="store.onSort"
                ...
                <Column field="provider.full_name" header="Service Provider" unstyle />
...
<script setup>
    ...
    onMounted(() => {
        nextTick(() => {
            emit.emit('pageName', 'Booking Management', [{ title: 'Bookings', routeName: "" }]);
        });
        store.getBookings();
    });
