<template>
      ...
      <div class="card mb-4">
        ...
        <div class="user-profile-header d-flex flex-column flex-sm-row text-sm-start text-center mb-4">
          <div class="flex-shrink-0 mt-n2 mx-sm-0 mx-auto">
            <img :src="props?.user?.profile_photo_url" alt="user image"
            ...
            <div
              ...
              <div class="user-profile-info">
                ...
                <ul
                  ...
                  <li v-if="props?.user?.location" class="list-inline-item"><i class="ti ti-map-pin"></i> {{ props?.user?.location }}</li>
                  <li class="list-inline-item"><i class="ti ti-calendar"></i> {{
                    formateDate(props?.user?.created_at) }}</li>
                </ul>
              </div>
          ...
          <ul class="list-unstyled mb-4 mt-3">
            <li class="d-flex align-items-center mb-3">
              <i class="ti ti-user"></i><span class="fw-bold mx-2">Full Name:</span> <span>{{
                props?.user?.full_name }}</span>
            </li>
            <li class="d-flex align-items-center mb-3">
              <i class="ti ti-check"></i><span class="fw-bold mx-2">Status:</span>
              <span class="badge cursor-pointer" :class="{
                        'bg-label-danger': (props?.user?.active == 0),
                        'bg-label-success': (props?.user?.active == 1),
                        }">{{ props?.user?.active == 1 ? "Active" : "Suspended" }}</span>
            </li>
            <!-- <li class="d-flex align-items-center mb-3">
      ...
      <div class="card mb-4">
        <div class="card-body">
          ...
          <ul class="list-unstyled mb-0">
            <li class="d-flex align-items-center mb-3">
              <i class="ti ti-check"></i><span class="fw-bold mx-2">Bookings Completed:</span> <span>135</span>
            </li>
            <li class="d-flex align-items-center mb-3">
              <Icon icon="material-symbols:pending-actions-rounded" width="20" height="20" />
              <span class="fw-bold mx-2">Bookings Pending:</span>
              <span>4</span>
            </li>
            <li class="d-flex align-items-center">
              <Icon icon="si:cancel-presentation-fill" width="20" height="20" />
              <span class="fw-bold mx-2">Bookings Cancelled:</span> <span>23</span>
            </li>
          </ul>
        </div>
      </div>
  ...
  <!-- <div v-if="activeTab == 'services'" class="">
    <div v-for="service in props?.user.services" class="col-lg-8 mb-4 col-md-12">
      <div class="card">
    ...
    <div class="card">
      <div class="w-75" v-if="props?.user?.services.length === 0">
        <h5 class="card-header">{{ props.user?.first_name }}'s Services</h5>
  ...
  <!-- <div v-if="activeTab == 'availabilities'" class="">
    <div class="card mb-4">
      <div v-if="props?.user?.availabilities?.length > 0">
        <div class="d-flex justify-content-between align-items-center">
          <h5 class="card-header">{{ props.user?.first_name }}'s availability</h5>
        ...
        <div class="table-responsive">
          <table class="table table-striped border-top">
            ...
            <tbody>
              <tr class="h-px-50" v-for="availability in props.user.availabilities">
      ...
      <div class="w-75" v-if="props?.user?.availabilities.length === 0">
        <h5 class="card-header">{{ props.user?.first_name }}'s availability</h5>
        <div class="d-flex justify-content-center">
          <img class="w-50" src="/public/admin_assets/assets/img/illustrations/No data-found.svg" alt="" srcset="">
        </div>
  ...
  <!-- User Bookings -->
   <div v-if="activeTab == 'bookings'" class="card p-4">
    <DataTable :value="store.list" lazy showGridlines @page="store.onPage" @sort="store.onSort"
                ...
                <Column field="provider.full_name" header="Provider Name">
                    <template #body="slotData">
                        <div class="d-flex flex-wrap">
                            <div class="avatar me-3">
                                <img :alt="slotData.data.full_name" :src="slotData.data.provider.profile_photo_url"
                                    class="rounded-circle">
                            </div>
                            <div>
                                <h6 class="mb-0">
                                    <Link :href="route('admin.service-providers.show', slotData.data?.provider?.id)">
                                    {{ slotData.data.provider.full_name }}
                                    </Link>
                                </h6>
                                <small>Member since {{dayjs(slotData.data.provider.created_at).format('MMM D, YYYY')}} </small>
                            </div>
                        </div>
                    </template>
                </Column>
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                <Link class="dropdown-item"
                                    :href="route('admin.service-providers.show', slotData.data?.provider?.id)"><i
                                    class="ti ti-eye me-1"></i> View Details</Link>
                                <Link class="dropdown-item"
                                    :href="route('admin.service-providers.edit', slotData.data?.provider?.id)"><i
                                    class="ti ti-edit me-1"></i> Edit</Link>
                                <button class="dropdown-item" @click="confirmDelete(slotData.data?.provider?.id)"><i
...
onMounted(() => {
  nextTick(() => {
    ...
    const breadcrumbs = [
      { title: 'Client', routeName: "admin.users.list" },
      {
        title: "Profile",
      }
    ];
