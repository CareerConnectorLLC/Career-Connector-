...
export const useBookingStore = defineStore('bookingStore', {
    ...
    actions: {
        async getBookings(page = this.pagination.current_page, perPage = this.pagination.per_page) {
            this.pagination.current_page = page;
            this.pagination.per_page = perPage;

            const response = await axios.post(route('admin.booking.list'), {
        ...
        onSearchQuery(searchValue) {
            ...
            this.searchTimeout = setTimeout(() => {
                if (searchValue.length > 2) {
                    if (index == -1 || !index) {                        
                        this.searchFilters.column = "client_name";
                ...
                this.getBookings(this.pagination.current_page);
        ...
        onPage(event) {
            this.getBookings(event.page + 1, event.rows);
        },
        onSort(event) {
        ...
        clearFilter() {
            ...
            this.filterModelVisible = false;
            router.visit(route('admin.booking.list'));
        },
        setFormData() {
